 ...
 */
// 形如静态资源的路径（字体、图片、脚本，以及扫描器探测）不可能是 Notion 文章。
// 不拦截的话它们会落到 catch-all 路由，每次都触发一次完整的全站抓取。
const STATIC_ASSET_PATH =
  /\.(?:woff2?|ttf|otf|eot|png|jpe?g|gif|svg|webp|avif|ico|bmp|css|m?js|map|json|xml|txt|php|env|ya?ml|sql|zip|gz|tar|bak|log|aspx?|jsp|cgi|pdf|mp4|webm|wasm)$/i
...
export async function resolvePostProps({ prefix, slug, suffix, locale, from }) {
  ...
  // 静态资源路径直接返回未找到，省掉一次完整的 Notion 抓取
  if (typeof lastSegment === 'string' && STATIC_ASSET_PATH.test(lastSegment)) {
    console.log(`[跳过] 静态资源路径，不抓取 Notion: ${fullSlug}`)
    return { post: null }
  }
  ...
  if (post) {
    ...
    // Notion 三次重试都失败时 getPageWithRetry 返回 null 而不是抛错，
    // 页面会被"成功"生成——有标题栏但正文为空——并被 ISR 缓存整整一小时。
    // 这里主动抛错：ISR 重新验证失败时 Next.js 会继续提供上一次成功的版本，
    // 首次生成失败则返回 500，两者都好过缓存一个空页面。错误会进入 Vercel 错误表。
    const blockCount = Object.keys(post?.blockMap?.block ?? {}).length
    if (blockCount === 0) {
      throw new Error(
        `[${taskId}] blockMap missing for "${fullSlug}" — refusing to cache a body-less page`
      )
    }
