...
export async function getConfigMapFromConfigPage(allPages) {
  ...
  // 补拉缺失的 block（Notion getPage 对 inline DB 可能截断行）
  const missingBlockIds = pageIds.filter(id => {
    const b = normalizePageBlock(block[id]?.value ?? block[id])
    return !b || !b.properties
  })
  if (missingBlockIds.length > 0 && viewIds?.length > 0) {
    console.log(`[Notion配置] 需要通过 getCollectionData 补拉 ${missingBlockIds.length} 条缺失的配置行`)
    try {
      const targetViewId = viewIds[0]
      const cv = collectionView?.[targetViewId]?.value?.value ?? collectionView?.[targetViewId]?.value
      const collectionData = await notionAPI.__call('getCollectionData', collectionId, targetViewId, cv, { limit: 999 })
      const extraBlocks = collectionData?.recordMap?.block || {}
      let addedCount = 0
      for (const [id, val] of Object.entries(extraBlocks)) {
        if (!block[id] || !normalizePageBlock(block[id]?.value ?? block[id])?.properties) {
          block[id] = val
          addedCount++
        }
      }
      console.log(`[Notion配置] 通过 getCollectionData 补充了 ${addedCount} 条 block`)
    } catch (e) {
      console.warn('[Notion配置] getCollectionData 补拉失败:', e.message)
    }
  }
  // 遍历用户的表格
