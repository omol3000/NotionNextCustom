...
export async function getConfigMapFromConfigPage(allPages) {
  ...
  blockKeys.forEach(id => {
    ...
    const pid = b?.parent_id || 'none'
    blockTypes[t] = (blockTypes[t] || 0) + 1
    // 检查是否有 page 类型 block 不在 initialPageIds 中
    if (t === 'page' && !initialPageIds.includes(id)) {
      console.log(`[DEBUG] 遗漏的page block: id=${id}, parent_id=${pid}, collectionId=${collectionId}`)
    }
  })
  console.log(`[DEBUG] block总数: ${blockKeys.length}, 类型分布: ${JSON.stringify(blockTypes)}, initialPageIds: ${initialPageIds.length}, collectionId: ${collectionId}`)

  // 补齐：page_sort/collectionQuery 可能截断，从 block 中补充遗漏的行
  const pageIdSet = new Set(initialPageIds)
  blockKeys.forEach(id => {
    if (pageIdSet.has(id)) return
    const b = normalizePageBlock(block[id]?.value ?? block[id])
    // 兼容：parent_id 可能指向 collectionId 或 configTableId
    if (b && b.type === 'page' && (b.parent_id === collectionId || b.parent_id === configTableId || b.parent_table === 'collection')) {
      pageIdSet.add(id)
    }
  })
  const pageIds = [...pageIdSet]
  if (pageIds.length > initialPageIds.length) {
    console.log(`[Notion配置] 从 block 补充了 ${pageIds.length - initialPageIds.length} 条被截断的配置行`)
  }
  if (pageIds?.length === 0) {
  ...
  console.log('[Notion配置] 已加载配置项数量:', Object.keys(notionConfig).length, '配置keys:', Object.keys(notionConfig).join(', '))
