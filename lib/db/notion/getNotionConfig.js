...
export async function getConfigMapFromConfigPage(allPages) {
  ...
  // 兜底：如果 getAllPageIds 未能获取到配置行，从 block 中直接提取
  if (pageIds?.length === 0) {
    const fallbackIds = Object.keys(block).filter(id => {
      const b = normalizePageBlock(block[id]?.value ?? block[id])
      return b && b.parent_id === collectionId && b.type === 'page'
    })
    if (fallbackIds.length > 0) {
      console.log('[Notion配置] 使用 block 兜底获取配置行，数量:', fallbackIds.length)
      pageIds = fallbackIds
    }
  }
  if (pageIds?.length === 0) {
