...
function generateCustomizeSlug(postProperties, NOTION_CONFIG) {
  ...
  allSlugPatterns.forEach((pattern, idx) => {
    if (pattern === '%year%' && (postProperties?.publishDate || postProperties?.publishDay)) {
      const formatPostCreatedDate = postProperties?.publishDate
        ? new Date(postProperties.publishDate)
        : new Date(postProperties.publishDay)
      if (!isNaN(formatPostCreatedDate.getTime())) {
