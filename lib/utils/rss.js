...
const createFeedContent = async post => {
  ...
  if (blockMap) {
    // 不要写回 post：这些对象来自 props.latestPosts，会被序列化进页面数据。
    // 写回会把每篇文章的完整 blockMap 塞进首页 payload。
...
export async function generateRss(props) {
  ...
  for (const post of latestPosts) {
    const feedDate = post?.publishDate ? new Date(post.publishDate) : new Date(post?.publishDay)
    feed.addItem({
      title: post.title,
      link: `${LINK}/${post.slug}`,
      description: post.summary,
      content: await createFeedContent(post),
      date: isNaN(feedDate.getTime()) ? new Date() : feedDate
    })
  }
